...
void update_camera(Camera* camera, double time)
{
    ...
    if ((clock() - camera->last_print_time) / CLOCKS_PER_SEC >= 1)
    {
        print_camera_facing(camera);
        camera->last_print_time = clock();
    }
...
void print_camera_facing(const Camera* camera)
{
    ...
    printf("Camera is facing direction: (x = %f,y = %f,z = %f)\n", direction.x, direction.y, direction.z);
}
